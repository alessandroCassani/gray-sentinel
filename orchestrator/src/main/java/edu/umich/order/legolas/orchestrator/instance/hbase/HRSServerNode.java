 ...
 *
 */
public final class HRSServerNode extends ServerNode {
    private static final Logger LOG = LoggerFactory.getLogger(HRSServerNode.class);

    private final String logFileName;

    public HRSServerNode(final MegaServer megaServer, final Orchestrator orchestrator,
        ...
        logFileName = "hbase-" + userName +"-" + serverId + "-regionserver-" + hostName + ".log";
    }
    ...
    public final synchronized void shutdown() throws Exception {
        final String pidFileName = "hbase-" + System.getProperty("user.name") +"-" + serverId + "-regionserver.pid";
    ...
    protected LogMonitor getLogMonitor() throws Exception {
        return new LogMonitor(this) {
            ...
            protected void handle(String line) {
                if (line.contains("STARTING executorService HRegionServer")) {
                    latch.countDown();
                }
            }
        };
