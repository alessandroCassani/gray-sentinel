 ...
 *
 */
public final class TaskManagerServerNode extends ServerNode {
    ...
    public TaskManagerServerNode(final MegaServer megaServer, final Orchestrator orchestrator,
    ...
    protected final String getLogFileName() {
        if (logFileName == null) {
            ...
            logFileName = "flink-" + userName + "-taskexecutor-0-" + hostName + ".log";
        }
        return logFileName;
    }
    ...
    private String getPidFileName() {
        if (pidFileName == null) {
            String userName = System.getProperty("user.name");
            pidFileName = "flink-" + userName + "-taskexecutor.pid";
        }
        return pidFileName;
    }
        ...
        try (final Scanner scanner = new Scanner(new File(pidFilePath))) {
            ...
            while (scanner.hasNextInt()) {
                ...
                if (pid != -1) {
                    ...
                    LOG.info("TaskManager with PID {} terminated", pid);
                }
            }
            pid = scanner.nextInt();
