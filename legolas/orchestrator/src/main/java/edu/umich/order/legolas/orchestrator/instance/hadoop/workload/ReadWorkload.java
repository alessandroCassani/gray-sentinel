 ...
 *
 */
public final class ReadWorkload extends Workload {
    private static final Logger LOG = LoggerFactory.getLogger(ReadWorkload.class);

    private final HadoopOrchestrator orch;
    public ReadWorkload(final HadoopOrchestrator orch, final int filenum, final int parallel, final int repeat) {
        super();
        this.orch = orch;
        for (int i = 0; i < filenum; i++) {
            for (int j = 0; j < parallel; j++) {
                ...
                clients.put(clientId, new ClientWorkload(orch, clientId,
                        new String[]{orch.workspacePathName + "/conf-1", "read", "/" + i, String.valueOf(repeat)}, repeat));
            }
        }
    }
    ...
    public void reportResult(final int phase) {
        ...
        LOG.info("Phase " + phase + " - read  :" + results);
    }
}
