...
function show_help() {
    echo "Usage: $0 [OPTION]"
    echo "Inject method-level failures in JVM applications using ChaosBlade"
    echo ""
    echo "Options:"
...
case $TYPE in
  delay)
    ...
    blade create docker jvm delay --container-id $CONTAINER_ID --class $METHOD --time $DELAY_MS --timeout $DURATION
  ;;
  ...
  exception)
    ...
    blade create docker jvm throwCustomException --container-id $CONTAINER_ID --class $METHOD --exception java.lang.RuntimeException --exception-message "$EXCEPTION_MSG" --timeout $DURATION
  ;;
