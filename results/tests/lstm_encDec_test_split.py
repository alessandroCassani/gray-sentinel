...
#-----------------------------------------------------------------------------------------------
#cambio di architettura. + profonda 
import numpy as np
import pandas as pd
...
def create_splits(df_engineered, selected_features, sequence_length=20, random_state=42):    
    ...
    sN_data = normal_features[:idx_60]
    vN1_data = normal_features[idx_60:idx_75]
    vN2_data = normal_features[idx_75:idx_90] 
    tN_data = normal_features[idx_90:]
...
def prepare_data(df_engineered, selected_features, sequence_length=20):
    ...
    scaler = RobustScaler()
    all_normal_data = np.concatenate([splits['sN'], splits['vN1'], splits['vN2'], splits['tN']])
    scaler.fit(all_normal_data)
    
    for key in ['sN', 'vN1', 'vN2', 'tN', 'vA', 'tA']:
        splits[key] = scaler.transform(splits[key])
        splits[key] = np.clip(splits[key], -3, 3)
...
def build_autoencoder(seq_len, n_features):
    ...
    encoded = Dense(16, activation='tanh', kernel_regularizer=l2(0.001))(encoded)
    
    decoded = RepeatVector(seq_len)(encoded)
    decoded = LSTM(32, activation='tanh', return_sequences=True,
                   kernel_regularizer=l2(0.001))(decoded)
    ...
    for p in percentiles:
        ...
        if np.sum(pred) > 0:
            ...
            if fpr > 0.30:
                ...
                base_score = f1 * 0.7 + (1 - fpr) * 0.3
                if precision >= 0.80 and recall >= 0.70 and fpr <= 0.25:
                    base_score *= 1.4
...
def smooth_predictions(predictions, window=3):
    ...
    for i in range(window, len(predictions)):
        if predictions[i] == 1:
            neighbors = predictions[i-window:i]
            if np.mean(neighbors) < 0.3:
                smoothed[i] = 0
    
    return smoothed

def calculate_weighted_error(predictions, sequences):
    weights = np.linspace(1.5, 0.8, sequences.shape[2])
    errors = np.abs(predictions - sequences)
    weighted_errors = errors * weights.reshape(1, 1, -1)
...
def train_and_evaluate(df_engineered, selected_features, sequence_length=20):
    ...
    threshold_predictions = autoencoder.predict(threshold_sequences, verbose=0)
    threshold_errors_standard = np.mean(np.abs(threshold_predictions - threshold_sequences), axis=(1, 2))
    threshold_errors_weighted = calculate_weighted_error(threshold_predictions, threshold_sequences)
    
    normal_mask = threshold_labels == 0
    sep_standard = np.mean(threshold_errors_standard[~normal_mask]) / np.mean(threshold_errors_standard[normal_mask])
    sep_weighted = np.mean(threshold_errors_weighted[~normal_mask]) / np.mean(threshold_errors_weighted[normal_mask])
    
    if sep_weighted > sep_standard:
        threshold_errors = threshold_errors_weighted
        use_weighted = True
    else:
        threshold_errors = threshold_errors_standard
        use_weighted = False
    
    normal_errors_thresh = threshold_errors[normal_mask]
    anomaly_errors_thresh = threshold_errors[~normal_mask]
    ...
    if use_weighted:
        test_errors = calculate_weighted_error(test_predictions, test_sequences)
    else:
        test_errors = np.mean(np.abs(test_predictions - test_sequences), axis=(1, 2))
    
    raw_predictions = (test_errors > best_threshold).astype(int)
    smoothed_predictions = smooth_predictions(raw_predictions, window=3)
    
    final_f1_raw = f1_score(test_labels, raw_predictions)
    final_f1_smoothed = f1_score(test_labels, smoothed_predictions)
    
    if final_f1_smoothed > final_f1_raw:
    ...
    if missed_failures <= 25 and false_alarm_rate <= 22:
        status = "✅ EXCELLENT: Both metrics in acceptable range"
    elif missed_failures <= 30 and false_alarm_rate <= 28:
        status = "✅ GOOD: Metrics acceptable for production"
    elif missed_failures <= 35 and false_alarm_rate <= 32:
        status = "⚠️  ACCEPTABLE: Usable but could be improved"
    else:
        status = "❌ NEEDS IMPROVEMENT: Too many missed failures or false alarms"
